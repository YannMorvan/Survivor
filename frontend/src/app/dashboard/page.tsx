...
export default function Dashboard() {
  ...
  const [selectedMeetingsPeriod, setSelectedMeetingsPeriod] =
    useState("30 jours");
  ...
  useEffect(() => {
    ...
    const fetchClientsData = async () => {
      try {
        const response = await sendPostRequest(route, {});
  ...
  useEffect(() => {
    ...
    const fetchClientsData = async () => {
      try {
        ...
        const validClients = data.customers.filter(
          (client: ClientData) => client.country && client.country !== "null"
        );
  ...
  const getProgressionLastMonth = (
    ...
    return (
      ((eventsByMonth.length - previousMonth.length) / previousMonth.length) *
      100
    );
  };

  const getEventsAvgDay = (
    events: Event[],
    month: number,
    year: number
  ): number => {
  ...
  const getProgressionAvgDay: (
    events: Event[],
    month: number,
    year: number
  ) => number = (events: Event[], month: number, year: number): number => {
    ...
    return previousMonth > 0
      ? ((eventsByMonth - previousMonth) / previousMonth) * 100
      : 0;
  };
  ...
  useEffect(() => {
    ...
    const fetchDataWeek = async () => {
      try {
        const response = await sendPostRequest(
          "http://localhost/last_week_events.php",
          {}
        );
        const data = JSON.parse(response);
    ...
    <div className="ml-6 sm:mr-6 mr-6 mb-5">
        ...
        <div className="mt-5">
          ...
          >
            <div className="flex justify-end">
              ...
              <p>{lastDays}</p>
              {isDropdownOpen ? (
                <ChevronDown size={16} className="ml-3 mt-0.5" />
              ) : (
                <ChevronRight size={16} className="ml-3 mt-0.5" />
              )}
            </div>
          </button>
          <div
          ...
          >
            ...
            >
              <li>
                <p
                  onClick={() => setLastDays("3 derniers mois")}
              ...
              <li>
                <p
                  onClick={() => setLastDays("30 derniers jours")}
                  className="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                >
                  30 derniers jours
                </p>
              ...
              <li>
                <p
                  onClick={() => setLastDays("7 derniers jours")}
                  className="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                >
                  7 derniers jours
                </p>
      ...
      <div className="lg:flex">
        <div className="bg-white rounded-sm p-5 mt-5 lg:w-7/12 border lg:mr-7">
          <div className="flex xs:justify-between">
            <div>
              <p className="font-medium md:text-md text-sm md:mr-0 mr-2">
                Aperçu des clients
              </p>
            </div>
            <div className="flex border rounded overflow-hidden">
              <p
                className={`text-xs border px-2 py-1 cursor-pointer ${
                  selectedPeriod === "7J" ? "bg-slate-100" : "bg-white"
                }`}
              ...
              <p
                className={`text-xs border px-2 py-1 cursor-pointer ${
                  selectedPeriod === "1M" ? "bg-slate-100" : "bg-white"
                }`}
              ...
              <p
                className={`text-xs border px-2 py-1 cursor-pointer ${
                  selectedPeriod === "3M" ? "bg-slate-100" : "bg-white"
                }`}
          ...
          <p className="text-slate-500 text-sm">
            Quelle période les clients ont rejoints
          </p>
        ...
        <div className="bg-white rounded-sm p-5 mt-5 lg:w-5/12 border">
          ...
          <p className="text-slate-500 text-sm">
            Nos evenements et leurs status
          </p>
          <div className="mt-7 flex justify-between lg:w-8/12">
            <div className="md:text-left text-center">
              <p className="text-slate-500 text-sm">Mensuel</p>
              <p className="mt-1 text-slate-900 text-xl">
                {augustEvents.length}
              </p>
              <div className="flex md:justify-normal justify-center">
              ...
              <div className="flex md:justify-normal justify-center">
                {progressionDaily < 0 ? (
                  <>
                    <ArrowDown size={12} className="text-red-700 mt-1.5" />
                    <p className="mt-1 text-xs text-red-700">
                      {progressionDaily.toPrecision(2)}%
                    </p>
                  </>
                ) : (
                  <>
      ...
      <div className="lg:flex">
          ...
          <div className="flex justify-between">
            ...
            <div>
              ...
              <div
                id="dropdown"
                className={`z-10 ${
                  mapIsDropdownOpen ? "block" : "hidden"
                } max-w-28 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 absolute mt-2`}
              >
                <ul
                  className="py-2 text-sm text-gray-700 dark:text-gray-200"
                  aria-labelledby="dropdownDefaultButton"
                >
                  <li>
                    <p
                      onClick={() => setSelectedMapPeriod("3 mois")}
                      className="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                    >
                      3 derniers mois
                    </p>
                  ...
                  <li>
                    <p
                      onClick={() => setSelectedMapPeriod("30 jours")}
                      className="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                    >
                      30 jours
                    </p>
                  ...
                  <li>
                    <p
                      onClick={() => setSelectedMapPeriod("7 jours")}
                      className="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                    >
                      7 jours
                    </p>
        ...
        <div className="bg-white rounded-sm p-5 mt-5 lg:w-5/12 border">
          <div className="flex justify-between">
            <div>
              <p className="font-medium md:text-md text-xs md:text-sm md:mr-0 mr-2">
                Sources principales des réunions
              </p>
            </div>
            <div>
              <button
              ...
              <div
                id="dropdown"
                className={`z-10 ${
                  isMeetingsDropdownOpen ? "block" : "hidden"
                } max-w-28 bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 absolute mt-2`}
              >
                <ul
                  className="py-2 text-sm text-gray-700 dark:text-gray-200"
                ...
                >
                  <li>
                    <p
                      onClick={() => setSelectedMeetingsPeriod("3 mois")}
                      className="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                    >
                      3 mois
                    </p>
                  ...
                  <li>
                    <p
                      onClick={() => setSelectedMeetingsPeriod("30 jours")}
                      className="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                    >
                      30 jours
                    </p>
                  ...
                  <li>
                    <p
                      onClick={() => setSelectedMeetingsPeriod("7 jours")}
                      className="cursor-pointer block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                    >
                      7 jours
                    </p>
